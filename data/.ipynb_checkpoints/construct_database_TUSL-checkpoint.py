...
def create_patient(conn, patient):
    sql = """INSERT INTO patients(patient_id, sexe, nr_of_sessions)
             VALUES(?,?,?)"""
...
def create_session(conn, session):
    sql = """INSERT INTO sessions(session_id, patient_id, electrode_setup, nr_of_tokens)
             VALUES(?,?,?,?)"""
...
def create_token(conn, token):
    sql = """INSERT INTO tokens(token_id, session_id, patient_id, url, file_name, file_path, sampling_freq, nr_of_samples, len_of_samples, recording_duration, nr_of_chs, eeg_chs, non_eeg_chs, diagnosis, number_of_windows)
             VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"""
...
## MAIN FUNCTION
# Connect with database
conn = create_connection("./eeg_recordings_TUSL.db")
...
with open('data/JSON_files/TUSL/TUSL_type.json', 'r') as f:
...
for electrode_setup in data_structure.keys():
    for patient_id in data_structure[electrode_setup].keys():            
        for session_id in data_structure[electrode_setup][patient_id].keys():
            for token_id in data_structure[electrode_setup][patient_id][session_id].keys():
                try:
                    edf_url = data_structure[electrode_setup][patient_id][session_id][token_id]
                    print(edf_url)
                    # Check if it is an .edf file
                    if edf_url[-4:] != ".edf":
                        continue
                    ...
                    print(edf_url_parts)
                    edf_file_name = edf_url_parts[13]
                    edf_file_path = "/" + "/".join(edf_url_parts[9:13]) + "/"
                    recording_setup = edf_url_parts[9]
                    recording_id = edf_url_parts[13][:-4]
                    print(f"recording_id({recording_id})")

                    # Add code to download file and save to specified path
                    Path("." + edf_file_path).mkdir(parents=True, exist_ok=True)
                    with open("." + edf_file_path + edf_file_name, 'wb') as f:
                        f.write(edf_bytes)
                    ...
                    token_information = edf_url_parts[13]
                    ...
                    print("pre diagnosis")
                    #diagnosis
                    if edf_file_name[:-4] in annotations.keys():
                        print("if statement")
                        diagnosis = ""
                        segments = annotations[edf_file_name[:-4]]
                        ...
                        token = (token_id, session_id[:4], patient_id, edf_url, edf_file_name, edf_file_path, sampling_freq, nr_of_samples, len_of_samples, recording_duration, nr_of_chs, str(eeg_chs), str(non_eeg_chs), diagnosis, "")
            ...
            nr_of_tokens = len(list(data_structure[electrode_setup][patient_id][session_id].keys()))
            session = (session_id[:4], patient_id, electrode_setup, nr_of_tokens)
        ...
        print("begin patient")
        nr_of_sessions = len(list(data_structure[electrode_setup][patient_id].keys()))

        if select_patients_by_id(conn, patient_id):
            update_patient_nr_of_sessions(conn, patient_id, nr_of_sessions)
        else:   
            patient = (patient_id, patient_sexe, nr_of_sessions)
            create_patient(conn, patient)
        conn.commit()
        print("einde patient")
    print("you finished bruh")
