...
def create_patient(conn, patient):
    sql = """INSERT INTO patients(patient_id, sexe, patient_train_or_test, nr_of_sessions)
             VALUES(?,?,?,?)"""
...
## MAIN FUNCTION
# Connect with database
conn = create_connection("./eeg_recordings_TUSZ.db")
...
with open('data/JSON_files/TUSZ/TUSZ_type.json', 'r') as f:
...
for train_or_test in data_structure.keys():
    for electrode_setup in data_structure[train_or_test].keys():
        for patient_id in data_structure[train_or_test][electrode_setup].keys():            
            for session_id in data_structure[train_or_test][electrode_setup][patient_id].keys():
                for token_id in data_structure[train_or_test][electrode_setup][patient_id][session_id].keys():
                    try:
                        edf_url = data_structure[train_or_test][electrode_setup][patient_id][session_id][token_id]
                        print(edf_url)
                        ...
                        edf_file_name = edf_url_parts[14]
                ...
                nr_of_tokens = len(list(data_structure[train_or_test][electrode_setup][patient_id][session_id].keys()))
            ...
            print("begin patient")
            nr_of_sessions = len(list(data_structure[train_or_test][electrode_setup][patient_id].keys()))

            if select_patients_by_id(conn, patient_id):
                update_patient_nr_of_sessions(conn, patient_id, nr_of_sessions)
            else:   
                patient = (patient_id, patient_sexe, train_or_test, nr_of_sessions)
