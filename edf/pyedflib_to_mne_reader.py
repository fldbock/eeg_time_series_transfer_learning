#!/usr/bin/env python
#
# file: $(NEDC_NFC)/util/python/nedc_pystream_mne/nedc_pystream_mne.py
#
# revision history:
...
# 
# usage:
#   nedc_pystream_mne -parameters parameters foo.edf
#
# options:
...
def nedc_print_usage():
     print("Usage: nedc_pystream_mne [-help] -p pfile.txt file1.edf")
     exit(-1)
#
# end of function
...
#
def nedc_print_help():
    print("name: nedc_pystream_mne")
    print("synopsis: nedc_pystream_mne [options] file")
    ...
    print("examples:")
    print("")
    print(" nedc_pystream_mne -p params.txt file1.edf")
    print("")
    print("  prints the signal data to stdout as floating point numbers")
    print("")
    print(" nedc_pystream_mne -help -param params.txt file1.edf")
    print("")
...
#
def nedc_print_vals(fsamp_a, sig_data_a, sig_times_a, labels_a):

     # check for no data
     #
     if len(sig_data_a) == int(0):
     ...
     nchans = len(sig_data_a)
     nsamples = min(map(len, sig_data_a))
     print("no. output channels = %3d" % (nchans))
     for i in range(nsamples):
          print("sample at %12.3f: " % (sig_times_a[i]))
          for j in range(len(sig_data_a)):
               print(" channel %3d (%15.15s, %12.4f): %12.8f µV" % \
                    (j, labels_a[j], fsamp_a[j], sig_data_a[j][i]))
...
def nedc_load_edf(fname_a):
    ...
    #
    num_chans = fp.info["nchan"]
    labels_tmp = fp.ch_names
    labels = [str(lbl.replace(' ', '')) for lbl in labels_tmp]

    # load each channel
    #
    sig_data, sig_times = fp.get_data(return_times=True)
    sig_data *= 10**6
    fsamp = [fp.info["sfreq"] for i in range(num_chans)]

	# plot topographical projections
	#
    fp.plot_projs_topomap(ch_type='eeg')
    
    # exit gracefully
    #
    return (fsamp, sig_data, sig_times, labels)
#
# end of function 
...
#
def nedc_select_channels(params_a, fsamp_a, sig_data_a, sig_times_a, labels_a):
    ...
    if KEYWORD_NULL in chan_list:
        return (fsamp_a, sig_data_a, sig_times_a, labels_a)
    ...
    for lbl in chan_list:
        ...
        if pos >= int(0):
            ...
            sig_data_sel.append(sig_data_a[pos])
            sig_times_sel = sig_times_a
    ...
    # exit gracefully
    #
    return fsamp_sel, sig_data_sel, sig_times_sel, labels_sel
#
# end of function 
...
#
def nedc_apply_montage(params_a, fsamp_a, sig_data_a, sig_times_a, labels_a):
     ...
     for i in range(len(montage)):
          ...
          #
          min_len = len(sig_data_a[pos1])
          if (pos2 >= int(0)):
               if len(sig_data_a[pos2]) < min_len:
                    min_len = len(sig_data_a[pos2])

          # copy the first signal
          #
          sig_data_mont.append(sig_data_a[pos1])
          sig_data_mont[i] = sig_data_mont[i][:min_len]
          
          sig_times_mont = sig_times_a
          sig_times_mont = sig_times_mont[:min_len]
          # difference the two signals if necessary
          #
          if pos2 >= int(0):
               for j in range(min_len):
                    sig_data_mont[i][j] -= sig_data_a[pos2][j]
     ...
     # exit gracefully
     #
     return (fsamp_mont, sig_data_mont, sig_times_mont, labels_mont)
#
# end of function
...
#
def main(edf_file, param_file):
    # loads the parameter file
    #
    params = nedc_load_parameters(param_file)
    if params == None:
    ...
    #
    fsamp, sig_data, sig_times, labels = nedc_load_edf(edf_file)

    # select channels from parameter file
    #
    fsamp_sel, sig_data_sel, sig_times_sel, labels_sel = nedc_select_channels(params,
                                                          fsamp, sig_data, sig_times, labels)

    # apply a montage
    #
    fsamp_mont, sig_data_mont, sig_times_mont, labels_mont = nedc_apply_montage(
         params, fsamp_sel, sig_data_sel, sig_times_sel, labels_sel)

    # print the values to stdout
    #
    nedc_print_vals(fsamp_mont, sig_data_mont, sig_times_mont, labels_mont)
...
if __name__ == "__main__":
    main("edf/example.edf", "edf/params_10_20_montage.txt")
#
# end of file 
