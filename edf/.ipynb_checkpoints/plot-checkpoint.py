...
for index, data in df.iterrows():
    ...
    if eeg_file_name[:-4] in annotations:
        ...
        for split in splits:
            ...
            window_width = 10
            windows = np.arange(0, data["recording_duration"], window_width / 4)

            number_of_windows = 0
            for index in windows:
                if int((index + window_width + start_time) * 256) <= int(stop_time * 256):
                    #print("         ", "Window is", index * 256, "to", (index + window_width) * 256)
                    eeg_prep_window_data = eeg_prep_data[:, int((start_time + index) * 256):int((index + window_width+ start_time) * 256)]
