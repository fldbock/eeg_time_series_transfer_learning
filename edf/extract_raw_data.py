...
h5f = h5py.File(f"/mnt/disks/data/files/TUAB_files/raw_data_TUAB.h5", 'w')
...
for index, data in df.iterrows():
    ...
    #print("   ", "Processing...")
    eeg = read_eeg(eeg_file_name, eeg_file_path='/mnt/disks/data/files/TUAB_files' + eeg_file_path)
    ...
    for index in windows:
        #print("      ", f"{int(index * 256)}/{int(data['recording_duration'] * 256)}")
        #print("         ", "Window is", index * 256, "to", (index + window_width) * 256)


        if int((index + window_width) * hz) <= data["recording_duration"] * hz:
            #print("         ", "Window is", index * 256, "to", (index + window_width) * 256)
            eeg_prep_window_data = eeg_prep_data[:, int(index * hz):int((index + window_width) * hz)]
    ...
    #
    print(eeg_file_name)
    h5f.create_dataset(f"{eeg_file_name}", data=eeg_prep_windows_data)#TODO:[-4:]
